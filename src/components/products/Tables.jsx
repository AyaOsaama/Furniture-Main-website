...
const Tables = () => {
  ...
  const chairVariants = currentVariants.filter(
  (product) =>
    product.category?.toLowerCase() === "chairs" || 
    product.category?.toLowerCase() === "كرسي" 
);
      ...
      <div className="lg:w-3/4 w-full mt-10">
        ...
        {hasLoaded && filteredVariants.length > 0 && (
          <div className="flex md:flex-row flex-col md:gap-0 gap-4 justify-between items-center mb-4">
            ...
            {t("showingProducts", {
  current: chairVariants.length,
  total: chairVariants.length,
})}
...
<ProductGrid
  hasLoaded={hasLoaded}
  currentVariants={chairVariants}
  filteredVariants={chairVariants}
